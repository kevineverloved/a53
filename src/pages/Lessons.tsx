  ...
  const fetchLessons = async () => {
    try {
      ...
      const {
        data: sections,
        error: sectionsError
      } = await supabase.from('sections').select('*').eq('license_type', licenseType).order('order_number');
      if (sectionsError) {
      ...
      const {
        data: lessons,
        error: lessonsError
      } = await supabase.from('lessons').select('*').eq('license_type', licenseType).order('order_number');
      if (lessonsError) {
    ...
    return <div className="min-h-screen bg-background p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <AnimatePresence mode="wait">
            {!showQuiz ? <StudyCard key="study-card" content={selectedSubsection.content[currentCardIndex]} currentIndex={currentCardIndex} totalCards={selectedSubsection.content.length} onNext={handleNextCard} /> : <Quiz key="quiz" questions={selectedSubsection.questions} onComplete={handleQuizComplete} lives={5} />}
          </AnimatePresence>
        </div>
      </div>;
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid md:grid-cols-12 gap-6">
            ...
            <Card className="glass">
              <CardContent className="p-4">
                ...
                <nav className="space-y-2">
                  {Object.keys(lessonData).map(section => <motion.button key={section} whileHover={{
                  scale: 1.02
                }} whileTap={{
                  scale: 0.98
                }} onClick={() => {
                  ...
                  setSelectedSubsection(null);
                }} className={`w-full text-left px-4 py-2 rounded-md transition-colors ${selectedSection === section ? 'bg-primary text-primary-foreground' : 'hover:bg-white/5'}`}>
                      {section}
                    </motion.button>)}
                </nav>
              </CardContent>
          ...
          x: 0
            ...
            {selectedSection ? <Card className="glass">
                <CardContent className="p-6">
                  <h2 className="text-2xl font-syne font-bold mb-6 text-gradient">{selectedSection}</h2>
                  <div className="grid md:grid-cols-2 gap-6">
                    {Object.entries(lessonData[selectedSection]).map(([subsectionTitle, lessons]) => <div key={subsectionTitle} className="glass rounded-lg p-4">
                        <h3 className="text-xl font-syne font-bold mb-4">{subsectionTitle}</h3>
                        <div className="space-y-4">
                          {lessons.map((lesson, index) => <motion.div key={index} whileHover={{
                      scale: 1.02
                    }} whileTap={{
                      scale: 0.98
                    }} className="glass hover:bg-white/10 rounded-lg p-4 cursor-pointer transition-colors" onClick={() => handleStartLesson(lesson)}>
